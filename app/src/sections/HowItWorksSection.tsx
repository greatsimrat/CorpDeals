...
const steps = [
  {
    ...
    description: 'Search your employer and preview relevant EPP and employee offer access.',
    icon: Search,
    color: 'bg-blue-500',
    lightColor: 'bg-blue-50',
    features: ['Employer lookup', 'Company suggestions', 'Quick matching'],
  },
  {
    number: '02',
    title: 'Verify in Seconds',
    description: 'Confirm your work email once with a secure verification step.',
    icon: ShieldCheck,
    color: 'bg-green-500',
    lightColor: 'bg-green-50',
    features: ['Work email verification', 'One-time code', 'Secure access'],
  },
  {
    number: '03',
    title: 'Unlock Exclusive Deals',
    description: 'Browse trusted offers across telecom, banking, travel, wellness, and more.',
    icon: Unlock,
    color: 'bg-purple-500',
    lightColor: 'bg-purple-50',
    features: ['Category browsing', 'Full offer details', 'Simple apply flow'],
  },
];
    ...
    >
      <div className="w-full px-6 lg:px-12">
        <div className="text-center mb-12">
          ...
          <h2 className="heading-2 text-corp-dark mb-4">HOW CORPDEALS WORKS</h2>
          <p className="body-text text-lg max-w-xl mx-auto">
            Access trusted Employee Purchase Program (EPP) and exclusive employee offers in three
            simple steps.
          </p>
        </div>
        ...
        >
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`absolute -top-4 left-8 ${step.color} text-white px-4 py-1 rounded-full font-mono text-sm font-bold`}
                >
                  STEP {step.number}
                </div>
                ...
                >
                  <Icon
                    className={`w-8 h-8 ${step.color.replace('bg-', 'text-').replace('500', '600')}`}
                  />
                </div>
                ...
                <ul className="space-y-2">
                  {step.features.map((feature) => (
                    <li key={feature} className="flex items-center gap-2">
                      <div
                        className={`w-5 h-5 ${step.lightColor} rounded-full flex items-center justify-center`}
                      >
                        <Zap
                          className={`w-3 h-3 ${step.color
                            .replace('bg-', 'text-')
                            .replace('500', '600')}`}
                        />
                      </div>
        ...
        <div ref={visualRef} className="max-w-4xl mx-auto">
          <div className="bg-white rounded-3xl shadow-card p-6 lg:p-8">
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4 w-full lg:w-auto">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-inter text-sm text-corp-gray">Start exploring</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <div className="flex items-center justify-center gap-2 text-sm text-corp-gray">
                ...
                <span className="px-3 py-1 bg-gray-100 rounded-full font-inter">
                  Amazon employee
                </span>
                <span>-&gt;</span>
                <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full font-inter">
                  Verified
                </span>
                <span>-&gt;</span>
                <span className="px-3 py-1 bg-corp-blue text-white rounded-full font-inter">
                  EPP and company deals unlocked
                </span>
              </div>
            </div>
          </div>
